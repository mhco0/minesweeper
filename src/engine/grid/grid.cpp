...
grid::grid(const int& width, const int height){
    ...
    if(not this->m_width or not this->m_height) return;
...
void grid::init(const grid_click_t& initial_click){
    ...
    for(size_t i = 0, j = 0; i < this->m_bombs and j < this->m_cells_sz; i++, j++){
    ...
    */
    ...
    for(size_t i = 0; i < this->m_height; i++){
        for(size_t j = 0; j < this->m_width; j++){
            ...
            if(this->m_cells[cell_index].type() == cell_type::BOMB) continue;
            else{
                // checks neightboards
                int neighboards[6] = {
                    (i - 1) * this->m_width + j,
                ...
                if(bombs_next){
                    this->m_cells[cell_index].set_type(cell_type::NUMERED);
